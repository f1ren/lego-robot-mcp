...
1. **Before planning any action sequence, explicitly describe what each camera shows:** gripper open or closed, arm high or low, object location relative to the robot, any clearance issues. Do not skip this step — motor position numbers alone are not sufficient. Only after this visual assessment should you list your planned moves.
   - **Gripper "open" means wide-open** (jaws fully apart). A partially-open gripper is NOT "open" — treat it as closed/partial, because it may not clear the object on approach and may fail to grab it once the jaws close. If the gripper is not wide-open, plan to open it fully before attempting a grasp.
...
4. **Directional language:** when describing the robot's heading or turning direction, always use **clockwise / counter-clockwise** (viewed from above) or **compass directions** (north / south / east / west). Never say the robot "turned left" or "turned right" — those terms are ambiguous depending on the observer's perspective. "Left wheel" and "right wheel" are fine as hardware identifiers (the gripper defines the robot's front), but motion outcomes must use CW/CCW or compass terms.
