...
1. **Before planning any action sequence, explicitly describe what each camera shows:** gripper open or closed, arm high or low, object location relative to the robot, any clearance issues. Do not skip this step — motor position numbers alone are not sufficient. Only after this visual assessment should you list your planned moves.
   - **If the target is not visible in `get_robot_state`:** call `scan_for_target` before `plan_pddl` or `navigate_to`. The scan rotates up to 180° CW in 30° steps using the front camera (YOLO only, no VLM), and leaves the robot pointing at the target when found. Only proceed to planning/navigation after the scan confirms the target is visible.
...
6. **For any multi-step task (navigate + grasp, navigate + place, or any combination), always call `plan_pddl` first** to get the grounded action sequence, then execute each step with the corresponding MCP tool. Do not manually construct a navigate → open-gripper → lower-arm → pick-up sequence — let the planner emit it. Reserve direct motor tools for single corrective actions (e.g. a small heading adjustment) after the plan is already running.
