...
- **Before executing a newly-proposed plan step, verify a matching tool doesn't already exist** (see the tool-catalog check in **Tool use and Code synthesis** below) — the VQA counselor reasons only over images and PDDL text and has no visibility into this server's tool catalog, so a proposed action name (e.g. `switch-on-light`) is not guaranteed to differ from an already-existing tool built for the same physical action (e.g. `click_button`). An unfamiliar plan-step name is not evidence that no tool exists for it.
...
3. **Before telling the user or a PDDL planner that no dedicated tool exists for an action, verify the full tool catalog — don't rely on what's already loaded in context.** A tool can be registered and available all session while going unmentioned, if its schema was never fetched and its name only appeared once, early, in a long list. (This is exactly what happened with `click_button` on 2026-07-17: it existed the whole session — fully implemented, purpose-built for "press a button/switch, then release" — but it was named once in the startup tool listing and never again, and Claude told the user no dedicated tool existed for pressing a wall switch.) Before concluding a tool is missing:
   - Run `ToolSearch` with a **keyword** query built from the action's verbs/nouns (e.g. `"button switch press"`) — a `select:` query naming tools you already expect to need will never surface a tool whose existence you don't already suspect.
   - Cross-check by grepping the MCP server source (`mcp_robot/server.py`) for `@mcp.tool()` definitions and docstrings matching the same keywords — the code is local and inspectable, so this is ground truth, not a guess.

   Only report "no tool for X" once both checks come back empty, and state what you checked (e.g. "searched ToolSearch and grepped server.py for button/switch/press — no match") so the claim is verifiable rather than asserted. This applies whether the "missing" action came from your own reasoning or from a PDDL plan step / VQA-proposed domain fix — see **Troubleshoot** above.
4. If no appropriate function or tool exists after that verification, the agentic coder should modify the MCP server code itself while testing. For instance, if you want the robot to grasp something, and the function does not exist, you should add the function to the MCP server code and test it. Same goes for moving forward, turning and etc.
5. Inspect the logs of the MCP server for debugging. The logs are available at `mcp_robot/logs/mcp_server.log`.
6. If the action verdict is NO or PARTIAL, or a step produces an unexpected null result (e.g. a scan detects nothing despite the target being expected in the scene), stop and check: is this a code bug, or a missing precondition/action in the domain? See the **Troubleshoot** section for next steps.
