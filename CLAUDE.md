...
6. **Always use `navigate_to` to move toward a target.** Never manually plan a `turn` + `drive` sequence for navigation — `navigate_to` handles turning, driving, and obstacle avoidance automatically. Reserve `turn` only for small heading corrections when already within arm's reach of the target.
