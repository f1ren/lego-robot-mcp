...
5. **Never turn with the arm down.** The arm in a lowered position interferes with turning and causes the wheels to slide instead of pivoting the robot. Always raise the arm before any `drive_degrees` turn call. Driving straight forward or backward with the arm down is allowed (e.g. to approach a ball).
