...
_VIDEO_PROMPT = (
    ...
    "grasp an object.\n"
    "GRASP READINESS: An object is ready to be grasped (gripper may close) only when it "
    "is at least touching BOTH the robot's front body AND the green forward-arrow overlay. "
    "If the object is visible but does not satisfy both conditions, set the Plan verdict "
    "to REPLAN — the robot must navigate closer before closing the gripper.\n"
...
_CLIP_PROMPT = (
    ...
    "GRASP READINESS: An object is ready to be grasped (gripper may close) only when it "
    "is at least touching BOTH the robot's front body AND the green forward-arrow overlay. "
    "If the object is visible but does not satisfy both conditions, do not close the "
