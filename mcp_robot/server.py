...
def click_button(
    ...
    """
    Press and release a button in one atomic motion, guaranteeing the button
    is released within press_duration_s + hold_duration_s + release_duration_s
    seconds — regardless of VLM validation latency.
    ...
    Args:
        ...
        hold_duration_s:    Seconds to hold the button down (motors stopped).
    ...
    """
    log.info(
        "[TOOL] click_button speed=%r press=%.2fs hold=%.2fs release=%.2fs",
        speed, press_duration_s, hold_duration_s, release_duration_s,
    )
    if not (config.SPEED_MIN <= abs(speed) <= config.SPEED_MAX):
        return _err(f"speed must be between {config.SPEED_MIN} and {config.SPEED_MAX} (abs).")
    total_s = press_duration_s + hold_duration_s + release_duration_s
    desc = (
        f"click_button speed={speed} press={press_duration_s}s "
        f"hold={hold_duration_s}s release={release_duration_s}s"
    )
    expected_str = expected if expected else (
        f"robot drives forward ~{press_duration_s}s (pressing button), "
        f"holds {hold_duration_s}s, then reverses ~{release_duration_s}s (releasing)"
    )
    return _with_change_analysis(
        desc, expected_str,
        lambda: robot_mod.click_button(speed, press_duration_s, hold_duration_s, release_duration_s),
        context=context,
        vqa_cameras={"droidcam"},
    )
